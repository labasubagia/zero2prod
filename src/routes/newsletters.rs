use actix_web::{http::header::{HeaderMap, self}, web, HttpRequest, HttpResponse, ResponseError};
use anyhow::Context;
...
async fn validate_credentials(credentials: Credentials, pool: &PgPool) ->Result<uuid::Uuid,PublishError> {
    let user_id = sqlx::query!(
        ...
        .fetch_optional(pool)
        .await.context("Failed to perform a query to validate auth credentials.")
